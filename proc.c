...
userinit(void)
{
  ...
  p->threadbool = 0; //Initially start with a process
...
growproc(int n)
{
  ...
  struct proc *p;
  for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
      if ((p->parent == proc && p->threadbool == 1) || (proc->parent == p && proc->threadbool == 1)) {
      p->sz = sz;
      }
  }
  return 0;
...
exit(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->parent == proc && p->threadbool == 1){
      p->killed = 1;
      pid = p->pid;
      release(&ptable.lock);
      join(pid);
...
clone(void(*fcn)(void*), void *arg, void *stack)
{
    ...
    //Check stack for page alignment and single page
...
int join(int pid) 
{
  struct proc *p;
  int havekids;
  //Called by a thread, invalid call
  if (proc->threadbool == 1) return -1;
